...
async def get_live_ticker(db: Session = Depends(get_db)):
    now_kst = datetime.now(KST)
    snapshot = get_ticker_snapshot(limit=1)
    active_eves = int(snapshot.get("active_eve_count") or 0)
    social_conversation_count = int(snapshot.get("social_conversation_count") or 0)

    active_room_ids: list[int] = []
    for rid, state in volatile_memory.items():
        if not isinstance(state, dict):
            continue
        ws = state.get("websocket")
        ws_connected = bool(ws and getattr(ws, "client_state", None) == WebSocketState.CONNECTED)
        status_online = str(state.get("status") or "").lower() == "online"
        if not (ws_connected or status_online):
            continue
        try:
            active_room_ids.append(int(rid))
        except Exception:
            continue

    room_rows = []
    if active_room_ids:
        room_rows = db.query(ChatRoom.id, ChatRoom.owner_id).filter(ChatRoom.id.in_(active_room_ids)).all()
    active_users = len({int(row.owner_id) for row in room_rows if row and row.owner_id is not None})
    live_user_conversation_count = len(room_rows)
    conversation_count = social_conversation_count + live_user_conversation_count
...
async def admin_get_persona_details(persona_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for r in rooms:
        owner = db.query(User).filter(User.id == r.owner_id).first()
        if owner:
            rel_pair = db.query(UserPersonaRelationship).filter(
                UserPersonaRelationship.user_id == owner.id,
                UserPersonaRelationship.persona_id == p.id,
            ).first()
            relationship_category = (
                rel_pair.relationship_category
                if rel_pair and rel_pair.relationship_category
                else (r.relationship_category or "친구")
            )
            relationship_summary = (
                rel_pair.relationship_summary_3line
                if rel_pair and rel_pair.relationship_summary_3line
                else (r.relationship_summary_3line or "")
            )
            eve_friends.append({
                "type": "USER",
                "name": owner.username,
                ...
                "interactions": "-",
                "relationship_summary_3line": relationship_summary,
            })
